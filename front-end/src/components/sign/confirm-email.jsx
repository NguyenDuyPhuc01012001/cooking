...
const ConfirmEmail = ({ email, handleShowUpdateInformation, handleClose }) => {
	...
	const sendVerificationEmail = (email) => {
		...
		axios
			.post(
				`${process.env.REACT_APP_API_URL}/api/v1/auth/sendVerificationEmail`,
				{
					email: email,
				}
			)
			.then((res) => {
				setIsFirst(false);
				alert(
					'Một email xác thực đã được gửi đến email của bạn. Vui lòng kiểm tra!'
				);
			})
			.catch((err) => {
				console.log(err);
		...
		<Fragment>
			<div className="container w-440 h-auto">
				...
				<div className="content m-top-42">
					...
					<div className="m-top-12 weight-400">
						Nếu đúng hãy chọn xác thực email, nếu sai email hãy chọn “Đổi email
						xác thực” ở phía dưới.
					</div>

					<button
						onClick={() => sendVerificationEmail(email)}
						className="resend-email-btn w-full"
					>
						Xác thực email
					</button>
					...
					<div className="footer-confirm-container m-top-24 padding-bot-24">
						<button
							...
							className="email-link weight-400 font-14"
						>
							Đổi email xác thực
						</button>
					</div>
			...
			<div className="container w-440 h-auto">
				...
				<div className="content m-top-42">
					<span className="weight-400">
						Để hoàn thành đăng kí tài khoản, xin vui lòng truy cập&nbsp;
					</span>
					<span className="email-link weight-400">{email}</span>
					<span className="weight-400">
						&nbsp;và xác thực email (Lưu ý kiểm tra Spam/Junk).
					</span>

					<div className="m-top-24 weight-400">
						Nếu bạn chưa nhận được mail xác thực, hãy chọn “Gửi lại email xác
						thực ”
					</div>
					...
					<div className="footer-confirm-container m-top-24 padding-bot-24">
						<span className=" weight-400 font-14">
							Bạn muốn đổi email đăng kí?{' '}
						</span>
						<button
							onClick={handleShowUpdateInformation}
							className="email-link weight-400 font-14"
