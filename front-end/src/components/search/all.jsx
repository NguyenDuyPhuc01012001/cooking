...
function All() {
	...
	const { dataAllRecipe } = useSelector((state) => state.commonReducer);
	...
	const handlePageClick = (event) => {
		...
		window.scrollTo(0, 0);
		history.push(findAllLink + `?page=` + event.selected);
	};
	/////////////////////////////////////////////////////////////////////////////
	useEffect(() => {
	...
	useEffect(() => {
		const url = `${process.env.REACT_APP_API_URL}/api/v1/recipe/getAll/user?page=${currentPage}`;
		async function getdataAllRecipe() {
			fetch(url, {
				method: 'GET',
			})
				.then((res) => res.json())
				.then((data) => {
					...
					// setdataAllRecipe(data.recipes);
					dispatch(LoadAllRecipe(data.recipes));
				})
