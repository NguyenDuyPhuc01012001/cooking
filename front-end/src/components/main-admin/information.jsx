...
function Information() {
	...
	const [checkValidState, dispatchValid] = useReducer(
		validReducer,
		validInitState
	);
	...
	const handleClick = () => {
		...
		if (name && slogan && overview) {
			...
			axios
				.post(
					'https://api.cloudinary.com/v1_1/nam-duong/image/upload',
					formData
				)
				.then((response) => {
	...
	const handleUploadImage = (e) => {
		...
		if (
			tempFile &&
			validFileType.includes(tempFile.type) &&
			tempFile.size <= maxFileSize
		) {
				...
				<div className="information-manager-container">
					<div className="information-content-container">
						...
						<div
							className={
								validName
									? 'information-management-input-field'
									: 'wrong-information-management-input-field'
							}
						>
							<input
